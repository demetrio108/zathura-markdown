...
girara_tree_node_t* markdown_document_index_generate(zathura_document_t* document, void* data, zathura_error_t* error) {
  ...
  /* get outline */
  fz_outline* outline = fz_load_outline(markdown_document->ctx, markdown_document->document);
  if (outline == NULL) {
  ...
  build_index(markdown_document->ctx, markdown_document->document, outline, root);

  /* free outline */
  fz_drop_outline(markdown_document->ctx, outline);
