...
char* markdown_page_get_text(zathura_page_t* page, void* data, zathura_rectangle_t rectangle, zathura_error_t* error) {
  markdown_page_t* markdown_page = data;

  if (page == NULL || markdown_page == NULL || markdown_page->text == NULL) {
    if (error != NULL) {
  ...
  char* ret = NULL;
#ifdef _WIN32
  ret = fz_copy_selection(markdown_page->ctx, markdown_page->text, a, b, 1);
#else
  ret = fz_copy_selection(markdown_page->ctx, markdown_page->text, a, b, 0);
#endif
...
girara_list_t* markdown_page_get_selection(zathura_page_t* page, void* data, zathura_rectangle_t rectangle,
  ...
  fz_quad* hits   = fz_malloc_array(markdown_page->ctx, MAX_QUADS, fz_quad);
  int num_results = fz_highlight_selection(markdown_page->ctx, markdown_page->text, a, b, hits, MAX_QUADS);
  ...
  fz_free(markdown_page->ctx, hits);
  g_mutex_unlock(&markdown_document->mutex);
